...
| `INFINO_MCP_SNIPPET_CHARS` | No | _off_ (full text) | Deployment default for search snippets: cut the text column of every search hit to this many characters (then `…`). Set it when rows are whole documents rather than chunks, so an agent scanning many hits doesn't pull every document into its context. A per-call `snippetChars` overrides it; `0` means full text. Only the searched text column is cut. |
...
Search hits carry the **full text column** by default, like other vector-store servers. When rows are whole documents rather than pre-chunked passages, set `INFINO_MCP_SNIPPET_CHARS` (for example `300`) so hits carry a snippet instead and an agent can scan many hits without pulling every document into its context; a per-call `snippetChars` overrides the deployment default, and `0` means full text. Only the searched text column is cut, never ids or other projected columns; read any single hit in full with `infino_sql`. Every search response also carries `score_kind`, stating whether its `score` is a distance (semantic: lower is closer) or a relevance (keyword and hybrid: higher is better).
